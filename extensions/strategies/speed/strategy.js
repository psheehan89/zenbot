    ...
    onPeriod: function (s, cb) {
      ...
      if (!s.in_preroll) {
        if (typeof s.period.macd_histogram === 'number' && typeof s.lookback[0].macd_histogram === 'number' && typeof s.period.srsi_K === 'number' && typeof s.period.srsi_D === 'number') {
          // Buy signal
          if ((k_buy && k_line_over_d_line && k_line_trending_up) || rsi_under_oversold) s.signal = 'buy'

          // Sell signal
          else if ((k_sell && !k_line_over_d_line && !k_line_trending_up) || rsi_over_overbought) s.signal = 'sell'
        }
      }
      cb()
    },
